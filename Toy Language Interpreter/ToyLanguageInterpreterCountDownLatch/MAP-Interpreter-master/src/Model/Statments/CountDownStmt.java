...
public class CountDownStmt implements IStmt{
    ...
    public PrgState execute(PrgState state) throws MyException {
        ...
        if(val.getType().equals(new IntType()))
        {
            ...
            if(latchTable.isDefined(foundIndex)) {
                int entry = latchTable.lookup(foundIndex);
                if (entry > 0)
                {
                    latchTable.update(foundIndex, entry - 1);
                }
                out.addElement(new IntValue(state.getId()));
            }
            else
    ...
    public MyIDictionary<String, Type> typecheck(MyIDictionary<String, Type> typeEnv) throws MyException {
        ...
        if (typeVar.equals(new IntType()))
            ...
            throw new MyException("CountDown: var is not of type int!");
    }
