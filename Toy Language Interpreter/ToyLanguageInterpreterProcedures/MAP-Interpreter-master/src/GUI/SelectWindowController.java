...
public class SelectWindowController implements Initializable {
    ...
    private ObservableList<IStmt> initList()
    {
        ...
        ArrayList<Exp> callParams = new ArrayList<>();
        callParams.add(new VarExp("v"));
        callParams.add(new VarExp("w"));
        ArrayList<Exp> callParams2 = new ArrayList<>();
        callParams2.add(new ArithExp(new VarExp("v"), new ValueExp(new IntValue(10)), '*'));
        callParams2.add(new VarExp("w"));

        IStmt ex9 = new CompStmt(new VarDeclStmt("v", new IntType()), new CompStmt(new AssignStmt("v", new ValueExp(new IntValue(2))),
                new CompStmt(new VarDeclStmt("w", new IntType()), new CompStmt(new AssignStmt("w", new ValueExp(new IntValue(5))),
                        new CompStmt(new CallStmt("sum", callParams2), new CompStmt(new PrintStmt(new VarExp("v")),
                                new CompStmt(new ForkStmt(new CallStmt("product", callParams)), new ForkStmt(new CallStmt("sum", callParams)))))))));
        ...
        selectButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override public void handle(ActionEvent e) {
                ...
                if(index == 9)
                {
                    ...
                    params.add("a");
                    params.add("b");
                    prgState.getProcTable().getContent().put("sum", new Pair<>(params, new CompStmt(new AssignStmt("v", new ArithExp(new VarExp("a"), new VarExp("b"), '+')), new PrintStmt(new VarExp("v")))));
                    prgState.getProcTable().getContent().put("product", new Pair<>(params, new CompStmt(new AssignStmt("v", new ArithExp(new VarExp("a"), new VarExp("b"), '*')), new PrintStmt(new VarExp("v")))));
